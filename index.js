...
function startShuffle() {
    disableButtons(true);
    const displayItems = document.querySelectorAll('.display-choose-items div img');
...
function shuffleImage(img) {
    ...
    }, 10); // Shuffle every 10 milliseconds, totaling 1000 ms = 1 second for 100 times
}
...
function compareResults(results) {
    const betsArray = [];
    const betImages = document.querySelectorAll('.choose-image-items .image-items-container');
...
function resetBets() {
    const betCounts = document.querySelectorAll('.choose-image-items .image-items-container p');
